...
peso_actual = st.number_input("Introduce tu peso actual (kg):", min_value=1.0, max_value=250.0, value=71.0, step=0.1)

# Operaciones matemáticas exactas basadas en la proporción de tus objetivos de Excel (71 kg -> 2756 kcal | 156.2g proteina)
objetivo_calorias = (2756 / 71) * peso_actual
objetivo_proteinas = (156.2 / 71) * peso_actual

col_obj1, col_obj2 = st.columns(2)
with col_obj1:
    st.info(f"🎯 **Meta Calorías:** {objetivo_calorias:.1f} kcal")
with col_obj2:
    st.info(f"🎯 **Meta Proteínas:** {objetivo_proteinas:.1f} g")
...
# --- BALANCE RESTANTE AL FINAL DEL DÍA ---
st.subheader("🏁 Balance Restante")
falta_calorias = objetivo_calorias - total_diario_cal
falta_proteinas = objetivo_proteinas - total_diario_prot

col_bal1, col_bal2 = st.columns(2)
with col_bal1:
    if falta_calorias >= 0:
        st.warning(f"⏳ Te faltan: **{falta_calorias:.1f} kcal**")
    else:
        st.success(f"✅ ¡Meta superada por! **{abs(falta_calorias):.1f} kcal**")

with col_bal2:
    if falta_proteinas >= 0:
        st.warning(f"⏳ Te faltan: **{falta_proteinas:.1f} g de prot**")
    else:
        st.success(f"✅ ¡Meta superada por! **{abs(falta_proteinas):.1f} g de prot**")
